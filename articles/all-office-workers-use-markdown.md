...
- プロンプトだけでなくドキュメントも可能な範囲内で Markdown 等のプレインテキストで記述した方がよい
...
- RAG でドキュメントを参照する際、プレインテキストの一種である Markdown は扱いやすい
  - .xlsx や .pptx や .docx など不自由なフォーマットのリッチテキストを RAG で扱う場合、プレインテキストに変換する手間が発生する
...
- Markdown などのプレインテキストは Git などのバージョン管理システムで変更履歴を管理するのが容易
...
LLM を有効活用し、十分なコンテキストを LLM に渡すには、
プロジェクトに新規に入ったメンバー用にオンボーディング資料を作成するのと同じように、
LLM がプロジェクトの背景や文脈を理解できるようなコンテキストを渡す必要がある
