...
- LLM を有効活用するために、開発者以外の職種の人間も Markdown を習得すべき
...
- Slack や Teams など多くのコミュニケーションツールは Markdown ライクのフォーマットを用いているため、Markdown のテキストを貼り付けるだけで書式が自動的に反映される
- Markdown などのプレーンテキストは Git などのバージョン管理システムで変更履歴を管理するのが容易
...
>
> XML では、ドキュメントは開始タグと終了タグのペアで構成されます。
...
>
> YAML では、ドキュメントは名前付きフィールドまたは名前のない箇条書きで構成され、階層レベルはインデントによって管理されます。
...
> また、プレインテキストであれば検索も容易になります。
>
> プレインテキストで記述しておけば、バージョン管理システムに管理させ、あらゆる変更の履歴を自動的に保存できるようになります。
...
> Markdown の枠組みの中でテキストを組み立てることが、書き手・読み手に低負荷なまま意思伝達を可能にする。
>
> ## リッチテキストの罠
>
...
> 編集ツールとして内部データをいじるのは想定されていないので、触ったしまった際にUIが読み込める保証はない。非可逆性がある。
>
> ## LLM とコンテキスト
>
> 人間同士のコミュニケーションで意味を制約しているのは、厳密なルールというよりも話者同士が共有するコンテキストにある。
> 日本人は特に同質的なコミュニティを前提にコンテキストを多く要求する傾向があり、主語を省略する文法にもそれが現れている。
>
...
仮に GitHub がサービス終了したとしても、
ローカルに Git の管理ファイルさえ残っていれば、
他の Git ホスティングサービスにアップロードするだけで移行は完了する

プレーンテキスト と Git を組み合わせると、プラットフォームの仕様に依存せずにドキュメントを管理できる
LLM がどうとかは抜きにしても、これだけでも非常に便利である

PowerPoint や Excel や Word といったソフトウェアは便利であり、
それ自体を否定するつもりは無いが、
LLM にとっては扱い辛いフォーマットでもある

LLM はもはや大半の人間よりも賢いのだから、LLM を活用しない手は無い
LLM の力を最大限に発揮させるには、
プロジェクトに新規に入ったメンバーにオンボーディング資料を渡すのと同じように、
プロジェクトの背景や文脈を理解できるようなコンテキストを LLM に渡す必要がある
そのコンテキストは、Markdown のようなプレーンテキストで記述されているのが望ましい

Markdown は、今までは開発者など一部の人間だけが使用する記法だったが、
